...
Lumina.CurrentTheme = deepCopy(Themes.Lumina)
...
local ConfigMethods = {
	...
	Load = function(Idx, Data, Path)
		if GetNestedValue(Lumina.Window.TabSections, Idx) then
			if GetNestedValue(Lumina.Window.TabSections, Idx) then
				for key, value in pairs(Data) do
					if table.find(ClassInterfacer[GetNestedValue(Lumina.Window.TabSections, Idx).Class], key) then
						GetNestedValue(Lumina.Window.TabSections, Idx):Set({ [key] = value })
					end
				end
			end
		end
	...
	UpdateOld = function(oldPath, newPath)
		...
		for i = 1, #list do
			local file = list[i]
			if file:sub(-#Lumina.FileSystem.FileExtension) == Lumina.FileSystem.FileExtension then
				local content = readfile(file)

				local pos = file:find(Lumina.FileSystem.FileExtension, 1, true)
				local start = pos
				...
				if char == "/" or char == "\\" then
					...
					if name ~= "options" then
						writefile(`{newPath}/{name}{Lumina.FileSystem.FileExtension}`, content)
					end
				end
		...
		local function set()
			pcall(task.spawn, function()
				if
					...
					and typeof(GetNestedValue(Lumina.CurrentTheme, themeKey)) == "Color3"
				then
					object[property] = ColorSequence.new({
						ColorSequenceKeypoint.new(0, GetNestedValue(Lumina.CurrentTheme, themeKey)),
						ColorSequenceKeypoint.new(1, GetNestedValue(Lumina.CurrentTheme, themeKey)),
					})
					return
				end

				object[property] = GetNestedValue(Lumina.CurrentTheme, themeKey)
			end)
		end
...
local function BlurModule(Frame : Frame)
	...
	local partRoot = Camera:FindFirstChild("Lumina Blur Elements") or Instance.new("Folder", Camera)
	partRoot.Name = "Lumina Blur Elements"
...
local function Hide(Interface, JustHide: boolean?, Notify: boolean?, Bind: string?)
	...
	if not isStudio and Lumina.Instance.MobileToggle.Visible then
	...
	if Notify then
		if Lumina.Instance.MobileToggle.Visible then
			Lumina:Notification({
...
local function AddToolTip(InfoStr, HoverInstance)
	...
	tooltip.ZIndex = 300
	tooltip.Parent = Lumina.Instance.Tooltips
...
local LuminaUI: ScreenGui = isStudio and script.Parent:WaitForChild("Lumina V2")
...
repeat
	if
		LuminaUI.Resources:FindFirstChild("Build")
		and LuminaUI.Resources.Build.Value == Lumina.InterfaceBuild
	then
		correctBuild = true
		break
	end
	...
	toDestroy, LuminaUI =
		LuminaUI,
		isStudio and script.Parent:FindFirstChild("Lumina V2") or game:GetObjects("rbxassetid://" .. modelId)[1]
...
LuminaUI.Name = (((getgenv and getgenv().InterfaceName) or LuminaUI.Name) or "Lumina Suite")
Lumina.Instance = LuminaUI
LuminaUI.Enabled = false
if not isStudio then
	pcall(function()
		LuminaUI.OnTopOfCoreBlur = true
	end)
end


if gethui then
	LuminaUI.Parent = gethui()
elseif syn and syn.protect_gui then
	syn.protect_gui(LuminaUI)

	LuminaUI.Parent = CoreGui
elseif not isStudio and CoreGui:FindFirstChild("RobloxGui") then
	LuminaUI.Parent = CoreGui:FindFirstChild("RobloxGui")
elseif not isStudio then
	LuminaUI.Parent = CoreGui
end
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == LuminaUI.Name and Interface ~= LuminaUI then
			Hide(Interface, true)
...
LuminaUI.MainWindow.Visible = false
LuminaUI.MainWindow.AnchorPoint = Vector2.zero
LuminaUI.MainWindow.Position = UDim2.fromOffset(
	Camera.ViewportSize.X / 2 - LuminaUI.MainWindow.Size.X.Offset / 2,
	((Camera.ViewportSize.Y / 2 - GuiInset) - LuminaUI.MainWindow.Size.Y.Offset / 2) - (GuiInset / 2)
)
LuminaUI:WaitForChild("Drag").Position = UDim2.new(
	0.5,
	0,
	0,
	((Camera.ViewportSize.Y / 2 - GuiInset) - LuminaUI.MainWindow.Size.Y.Offset / 2)
	- (GuiInset / 2)
		+ LuminaUI.MainWindow.Size.Y.Offset
		+ 10
)



local mainWindow: Frame = LuminaUI.MainWindow
local Resources = LuminaUI.Resources
...
if UserInputService.TouchEnabled then
	LuminaUI.Notifications.Interactable = false
end
...
if PlayerGui:FindFirstChild("TouchGui") then
	...
	local function check()
		if jumpButton and jumpButton.Visible then
			LuminaUI.Notifications.Position = UDim2.new(1, -20, 1, -(24 + jumpButton.AbsoluteSize.Y))
		else
			LuminaUI.Notifications.Position = UDim2.new(1, -20, 1, -20)
		end
	end
...
function Lumina:OnDestroy(func)
	Lumina.DestroyFunction = func
end
...
LuminaUI.Destroying:Connect(function()
	pcall(Lumina.DestroyFunction)
	for i, v in pairs(connections) do
		v:Disconnect()
	end
	if Lumina.Window then
		for _, tabSection in pairs(Lumina.Window.TabSections) do
			tabSection:Destroy()
		end
	end
	for i, v in pairs(Lumina) do
		v = nil
	end
	if Camera:FindFirstChild("Lumina Blur Elements") then
		for _, blur in pairs(Camera:FindFirstChild("Lumina Blur Elements"):GetChildren()) do
			blur:Destroy()
		end
...
function Lumina:Notification(data)
	...
	task.spawn(function()
		...
		newNotification.Parent = LuminaUI.Notifications
		newNotification.LayoutOrder = #LuminaUI.Notifications:GetChildren()
		...
		newNotification.Size =
			UDim2.new(1, 0, 0, -LuminaUI.Notifications:FindFirstChild("UIListLayout").Padding.Offset)
		...
		if data.Duration >= 0 then
			...
			pcall(function()
				if not Lumina.NotificationsOpen then
					...
					Tween(
						newNotification,
						{
							Size = UDim2.new(
								...
								-90,
								0,
								-LuminaUI.Notifications:FindFirstChild("UIListLayout").Padding.Offset
							),
						},
						function()
							newNotification.Visible = false
...
function Lumina:CreateWindow(WindowSettings)
	...
	then
		warned = true
		warn("Lumina | Build Mismatch")
		warn(
			"Lumina may run into issues as it seems you are running an incompatible interface version ("
				.. (LuminaUI.Resources:FindFirstChild("Build") and LuminaUI.Resources:FindFirstChild("Build").Value or "No Build")
				.. "). of Lumina\n\nThis version of Lumina is intended for interface build "
				.. Lumina.InterfaceBuild
				.. ".\nTry rerunning the script. If the issue persists, join our discord for support."
		)
		pcall(function()
			Lumina:Notification({
				Title = "Lumina - Build Mistmatch",
				Content = "Lumina may run into issues as it seems you are running an incompatible interface version ("
					.. (LuminaUI.Resources:FindFirstChild("Build") and LuminaUI.Resources:FindFirstChild("Build").Value or "No Build")
					.. "). of Lumina\n\nThis version of Lumina is intended for interface build "
					.. Lumina.InterfaceBuild
					.. ". \nTry rerunning the script. If the issue persists, join our discord for support.",
	...
	Lumina.FileSystem.AutoloadConfigPath = `{Lumina.FileSystem.Folder}/{folderpath}/configs/`
	if WindowSettings.FileSettings.ThemesInRoot then
		Lumina.FileSystem.AutoloadThemePath = `{Lumina.FileSystem.Folder}/{root}/themes/`
	else
		Lumina.FileSystem.AutoloadThemePath = `{Lumina.FileSystem.Folder}/{folderpath}/themes/`
	end

	Lumina.FileSystem:BuildFolderTree(WindowSettings.FileSettings)
	...
	do
		...
		pcall(function()
			...
			AcrylicObject2.AddParent(LuminaUI.MobileToggle)
			AcrylicObject2.Frame.Parent = LuminaUI.MobileToggle
		...
		acrylicEvent.Event:Connect(function()
			...
			if mainAcrylic then
				...
				Tween(LuminaUI.MobileToggle.Backdrop, { BackgroundTransparency = 0.5 })
				Tween(LuminaUI.MobileToggle.Backdrop.UIStroke, { Transparency = 0.5 })
				...
				Tween(LuminaUI.MobileToggle.Backdrop, { BackgroundTransparency = 0 })
				Tween(LuminaUI.MobileToggle.Backdrop.UIStroke, { Transparency = 0 })
		...
		LuminaUI.MobileToggle.Image = WindowSettings.Icon ~= nil and "rbxassetid://" .. WindowSettings.Icon
		...
		LuminaUI.Drag.Position = UDim2.new(
			0.5,
			0,
			0,
			((Camera.ViewportSize.Y / 2 - GuiInset) - LuminaUI.MainWindow.Size.Y.Offset / 2)
		...
		do
			ThemeMethods.bindTheme(LuminaUI.MobileToggle.Backdrop, "BackgroundColor3", "Backgrounds.Dark")
			ThemeMethods.bindTheme(LuminaUI.MobileToggle.Backdrop.UIStroke, "Color", "Foregrounds.Dark")
			for _, shadow in pairs(LuminaUI.MobileToggle.Backdrop.DropShadowHolder:GetChildren()) do
			...
			ThemeMethods.bindTheme(LuminaUI.Drag.DragCosmetic, "BackgroundColor3", "Foregrounds.Light")
		...
		task.spawn(function()
			if WindowSettings.LoadingEnabled then
				...
				LuminaUI.Drag.Visible = true
				LuminaUI.MobileToggle.Visible = UserInputService.TouchEnabled
				...
				title.Text = WindowSettings.LoadingSettings and WindowSettings.LoadingSettings.Title
					or "Lumina Suite"
				versionLabel.Text = title.Text == "Lumina Suite" and Release or `Lumina UI {Release}`
		...
		makeDraggable(mainWindow.Content.Topbar, mainWindow, LuminaUI.Drag)
		makeDraggable(mainWindow.Sidebar, mainWindow, LuminaUI.Drag)
		makeDraggable(LuminaUI.MobileToggle, LuminaUI.MobileToggle, nil)
		if LuminaUI.Drag then
			makeDraggable(LuminaUI.Drag.Interact, mainWindow, LuminaUI.Drag, true, nil, LuminaUI.Drag)
		end
	...
	function Lumina.Window:PromptDialog(ModalSettings)
		...
		if Modal.Values.Type == 1 then
			for Key, Action in pairs(Modal.Values.Actions) do
				...
				ActionButton.MouseButton1Click:Connect(function()
					close(function()
						...
						if not Success then
							warn(`Lumina Suite - Callback Error | Dialog {Modal.Values.Name}`)
							print(Response)
			...
			for _, Action in pairs(Modal.Values.Actions) do
				...
				ActionInput.MouseEnter:Connect(function()
					Tween(ActionInput.UIStroke, { Color = Lumina.CurrentTheme.Foregrounds.DarkHover })
				end)
				ActionInput.MouseLeave:Connect(function()
					Tween(ActionInput.UIStroke, { Color = Lumina.CurrentTheme.Foregrounds.Dark })
				end)
	...
	function Lumina.Window:CreateHomeTab(TabSettings)
		...
		if not prebuiltTabSection then
			prebuiltTabSection = Lumina.Window:CreateTabSection()
		...
		local function Activate() 
			...
			Tween(Tab.Instances.Button.Icon, { ImageColor3 = Lumina.CurrentTheme.Foregrounds.Light })
			Tween(Tab.Instances.Button.Header, { TextColor3 = Lumina.CurrentTheme.Foregrounds.Light })
			Tab.Instances.Button.Icon.AccentBrighter.Enabled = true
			Tab.Instances.Button.Header.AccentBrighter.Enabled = true

			for i, v in pairs(Lumina.Window.TabSections) do
			...
			for _, OtherTabSection in pairs(navigation:GetChildren()) do
				for _, OtherTab in pairs(OtherTabSection:GetChildren()) do
					if OtherTab.ClassName == "Frame" and OtherTab ~= Tab.Instances.Button then
						...
						Tween(OtherTab.Icon, { ImageColor3 = Lumina.CurrentTheme.Foregrounds.Medium })
						Tween(OtherTab.Header, { TextColor3 = Lumina.CurrentTheme.Foregrounds.Medium })
		...
		Tab.Instances.Button.MouseLeave:Connect(function()
			Tab.Hover = false
			if not Tab.Active then
				Tween(Tab.Instances.Button.Icon, { ImageColor3 = Lumina.CurrentTheme.Foregrounds.Medium })
				Tween(Tab.Instances.Button.Header, { TextColor3 = Lumina.CurrentTheme.Foregrounds.Medium })
			end
		end)
	...
	function Lumina.Window:CreateTabSection(Name: string, Visible)
		...
		function TabSection:Set(NewName)
			...
			TabSection.Instance.Header.Text = Name
			TabSection.Instance.Name = "TAB_SECTION_" .. Name
			Lumina.Window.TabSections[Name] = TabSection
		end
		...
		function TabSection:CreateCustomTab(TabSettings, TabIndex)
			...
			if Lumina.Window.CurrentTab == nil then
			...
			function Tab:Set(NewTabSettings)
				...
				Lumina.Window.TabSections[Name].Tabs[TabIndex].Values = Tab.Values
			end
			...
			Tab.Instances.Button.Parent = Lumina.Window.TabSections[Name].Instance
			Lumina.Window.TabSections[Name].Tabs[TabIndex] = Tab
			return Lumina.Window.TabSections[Name].Tabs[TabIndex]
		end
		...
		function TabSection:CreateTab(TabSettings, TabIndex)
			...
			function Tab:CreateGroupbox(GroupboxSettings, GroupIndex)
				...
				function Groupbox:Set(NewGroupboxSettings)
					...
					Lumina.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex].Values =
						NewGroupboxSettings
				end
				...
				function Groupbox:CreatePrimaryButton(ElementSettings) 
					...
					function Element:Set(NewElementSettings)
						...
						Lumina.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex].Elements[ElementSettings.Name] = ElementSettings
						...
						Element.Instance["PART_Backdrop"].Header.Icon.Image = ElementSettings.Icon ~= nil and "rbxassetid://" .. Element.Values.Icon or ""
						Lumina.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex].Elements[ElementSettings.Name].Values = ElementSettings
					end
					...
					Element.Instance.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							warn("Lumina Suite | "..ElementSettings.Name.." Callback Error")
					...
					Lumina.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex].Elements[ElementSettings.Name] = Element
					return Lumina.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex].Elements[ElementSettings.Name]
				end
					...
					UserInputService.InputBegan:Connect(function(input, processed)
						...
						if CheckingForKey then
							...
							if input.UserInputType == Enum.UserInputType.Keyboard then
								if input.KeyCode ~= Enum.KeyCode.Unknown and input.KeyCode ~= Enum.KeyCode[Lumina.WindowKeybind] then
				...
				function Groupbox:CreateButton(ElementSettings, Index)
					...
					task.spawn(function()
						...
						for i, ElementInstance in pairs(Instances) do
							...
							ElementInstance.Interact.MouseButton1Click:Connect(function()
								...
								if not Success then
									...
									warn(`Lumina Suite - Callback Error | {Element.Values.Name} ({Index})`)
									print(Response)
									if WindowSettings.NotifyOnCallbackError then
										Lumina:Notification({
						...
						function Element:Set(NewElementSettings, NewIndex)
							...
							Lumina.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex].Elements["BUTTON_" .. Index].Values =
								Element.Values
						end
					...
					Lumina.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex].Elements["BUTTON_" .. Index] =
						Element
					return Lumina.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex].Elements["BUTTON_" .. Index]
				end
				...
				function Groupbox:CreateToggle(ElementSettings, Index)
					...
					task.spawn(function()
						...
						local tooltips = {}
						local knobcolor = Lumina.CurrentTheme.Foregrounds.Medium
							...
							if bool then
								...
								Tween(
									Instances.Style2.Switch.Knob,
									{
										...
										BackgroundColor3 = Lumina.CurrentTheme.Foregrounds.Active,
										BackgroundTransparency = 0,
									}
								)
						...
						for i, ElementInstance in pairs(Instances) do
							...
							if ElementInstance:FindFirstChild("Checkbox") then
								...
								do
									...
									if not Success then
										...
										warn(
											`Lumina Suite - Callback Error | {Element.Values.Name} ({Index})`
										)
										print(Response)
										if WindowSettings.NotifyOnCallbackError then
											Lumina:Notification({
							...
							elseif ElementInstance.Switch then
								...
								ElementInstance.Switch.MouseEnter:Connect(function()
									knobcolor = Lumina.CurrentTheme.Foregrounds.MediumHover
								...
								ElementInstance.Switch.MouseLeave:Connect(function()
									knobcolor = Lumina.CurrentTheme.Foregrounds.Medium
									if not Element.Values.CurrentValue then
										Tween(ElementInstance.Switch, { BackgroundColor3 = knobcolor })
										Tween(ElementInstance.Switch.Knob, { BackgroundColor3 = knobcolor })
										Tween(ElementInstance.Switch.UIStroke, { Color = knobcolor })
						...
						function Element:Set(NewElementSettings, NewIndex)
							...
							Lumina.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex].Elements[Index].Values =
								Element.Values
						end
					end)
					...
					Lumina.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex].Elements[Index] = Element
					return Lumina.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex].Elements[Index]
				end
				...
				function Groupbox:CreateDivider()
					...
					Lumina.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex].Elements["Divider_" .. Divider.ID] =
						Divider
					return Lumina.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex].Elements["Divider_" .. Divider.ID]
				end
					...
					task.spawn(function()
						...
						Element.Instance.MouseEnter:Connect(function()
							Tween(
								...
								{ Color = Lumina.CurrentTheme.Foregrounds.DarkHover }
							)
						end)
						Element.Instance.MouseLeave:Connect(function()
							Tween(
								Element.Instance.PART_Backdrop.UIStroke,
								{ Color = Lumina.CurrentTheme.Foregrounds.Dark }
							)
						end)
				...
				function Groupbox:CreateLabel(ElementSettings, Index)
					...
					function Element:AddBind(NestedSettings, NestedIndex, Parent, ParentIndex)
						...
						task.spawn(function()
							...
							task.delay(0.2, function()
								NestedElement.Instance.Text = NestedElement.Values.CurrentValue == "No Bind"
									and '<font color="rgb(' .. tostring(
										math.floor(Lumina.CurrentTheme.Foregrounds.Medium.R * 255 + 0.5)
									) .. "," .. tostring(
									math.floor(Lumina.CurrentTheme.Foregrounds.Medium.G * 255 + 0.5)
								) .. "," .. tostring(
									math.floor(Lumina.CurrentTheme.Foregrounds.Medium.B * 255 + 0.5)
								) .. ')">No Bind</font>'
										...
										if NestedElement.Values.WindowSetting then
											if input.KeyCode ~= Enum.KeyCode.Unknown then
												...
												local Success, Response = pcall(function()
													...
													Lumina.WindowKeybind = tostring(NewKeyNoEnum)
												end)

												if not Success then
													Parent.Instance.Header.Text = "Callback Error"
													warn(
														`Lumina Suite - Callback Error | {Element.Values.Name} ({Index} {NestedIndex})`
													)
													print(Response)
													if WindowSettings.NotifyOnCallbackError then
														Lumina:Notification({
												...
												Lumina.WindowKeybind = nil
											end
										elseif input.UserInputType == Enum.UserInputType.Keyboard then
											if
												input.KeyCode ~= Enum.KeyCode.Unknown
												and input.KeyCode ~= Enum.KeyCode[Lumina.WindowKeybind]
											then
												local SplitMessage = string.split(tostring(input.KeyCode), ".")
												local NewKeyNoEnum = SplitMessage[3]
												NestedElement.Instance.Text = tostring(NewKeyNoEnum)
												...
												NestedElement.Instance:ReleaseFocus()
											elseif input.KeyCode == Enum.KeyCode[Lumina.WindowKeybind] then
												NestedElement.Instance.Text = NestedElement.Values.CurrentValue
													== "No Bind"
													and '<font color="rgb(' .. tostring(
														math.floor(
															Lumina.CurrentTheme.Foregrounds.Medium.R * 255 + 0.5
														)
													) .. "," .. tostring(
													math.floor(
														Lumina.CurrentTheme.Foregrounds.Medium.G * 255 + 0.5
													)
												) .. "," .. tostring(
													math.floor(
														Lumina.CurrentTheme.Foregrounds.Medium.B * 255 + 0.5
													)
												) .. ')">No Bind</font>'
													or NestedElement.Values.CurrentValue
												NestedElement.Instance:ReleaseFocus()
							...
							local Success, Response = pcall(function()
								...
								if NestedElement.Values.WindowSetting then
									Lumina.WindowKeybind = tostring(NestedElement.Values.CurrentValue)
								end
							end)
							...
							function NestedElement:Set(NewNestedSettings, NewNestedIndex)
								...
								Lumina.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex].Elements[ParentIndex].NestedElements[NestedIndex].Values =
									NestedElement.Values
							end
						end)

						Lumina.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex].Elements[ParentIndex].NestedElements[NestedIndex] =
							NestedElement
						return Lumina.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex].Elements[ParentIndex].NestedElements[NestedIndex]
					end
					...
					function Element:AddColorPicker(NestedSettings, NestedIndex, Parent, ParentIndex)
						...
						task.spawn(function()
							...
							NestedElement.Instances[2].Parent = LuminaUI.PopupOverlay
								...
								TabButton.MouseButton1Click:Connect(function()
									for _, OtherTabButton in pairs(NestedElement.Instances[2].TabSelector:GetChildren()) do
										...
										Tween(
											...
											{
												BackgroundTransparency = 1,
												TextColor3 = Lumina.CurrentTheme.Foregrounds.Medium,
											}
										)
						...
						task.spawn(function()
							...
							local function Activate(option)
								pcall(function()
									...
									Tween(option.header, { TextColor3 = Lumina.CurrentTheme.Foregrounds.Light })
									Tween(
										option.UIPadding,
							...
							local function Deactivate(option)
								pcall(function()
									Tween(option, { BackgroundTransparency = 1 })
									Tween(option.header, { TextColor3 = Lumina.CurrentTheme.Foregrounds.Medium })
									Tween(
										option.UIPadding,
									...
									optioninstance.MouseEnter:Connect(function()
										if optioninstance:GetAttribute("Active") == false then
											...
											Tween(
												...
												{ TextColor3 = Lumina.CurrentTheme.Foregrounds.Light }
											)
										end
									end)
				...
				function Groupbox:CreateParagraph(ElementSettings, Index)
					...
					task.spawn(function()
						...
						function Element:Set(NewElementSettings, NewIndex)
							...
							Lumina.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex].Elements[NewIndex].Values =
								ElementSettings
						end
				...
				Lumina.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex] = Groupbox
				return Lumina.Window.TabSections[Name].Tabs[TabIndex].Groupboxes[GroupIndex]
			end
			...
			function Tab:BuildThemeGroupbox(Column, Style, ButtonsCentered)
				...
				local themesPath = WindowSettings.FileSettings.ThemesInRoot
					and `{Lumina.FileSystem.Folder}/{root}/themes`
					or `{Lumina.FileSystem.Folder}/{folderpath}/themes`
				...
				local customThemes = not isStudio and Lumina.FileSystem:RefreshConfigList(themesPath) or {}
				...
				do
					do
						...
						local cp = colorpickers.bg:AddColorPicker({
							IgnoreConfig = true,
							CurrentValue = Lumina.CurrentTheme.Backgrounds.Dark,
							Callback = function(c)
								debounce = true
								Lumina.CurrentTheme.Backgrounds.Dark = c
								themeEvent:Fire()
								task.wait(6 / 60)
								debounce = false
							end,
						...
						themeEvent.Event:Connect(function()
							if not debounce then
								cp:Set({ CurrentValue = Lumina.CurrentTheme.Backgrounds.Dark }, nil, true)
							end
						end)
					end
					do
						...
						local cp = colorpickers.bg:AddColorPicker({
							IgnoreConfig = true,
							CurrentValue = Lumina.CurrentTheme.Backgrounds.Medium,
							Callback = function(c)
								debounce = true
								Lumina.CurrentTheme.Backgrounds.Medium = c
								themeEvent:Fire()
								task.wait(6 / 60)
								debounce = false
							end,
						...
						themeEvent.Event:Connect(function()
							if not debounce then
								cp:Set({ CurrentValue = Lumina.CurrentTheme.Backgrounds.Medium }, nil, true)
							end
						end)
					end
					do
						...
						local cp = colorpickers.bg:AddColorPicker({
							IgnoreConfig = true,
							CurrentValue = Lumina.CurrentTheme.Backgrounds.Light,
							Callback = function(c)
								debounce = true
								Lumina.CurrentTheme.Backgrounds.Light = c
								themeEvent:Fire()
								task.wait(6 / 60)
								debounce = false
							end,
						...
						themeEvent.Event:Connect(function()
							if not debounce then
								cp:Set({ CurrentValue = Lumina.CurrentTheme.Backgrounds.Light }, nil, true)
							end
						end)
					end
					do
						...
						local cp = colorpickers.bg:AddColorPicker({
							IgnoreConfig = true,
							CurrentValue = Lumina.CurrentTheme.Backgrounds.Groupbox,
							Callback = function(c)
								debounce = true
								Lumina.CurrentTheme.Backgrounds.Groupbox = c
								themeEvent:Fire()
								task.wait(6 / 60)
								debounce = false
							end,
						...
						themeEvent.Event:Connect(function()
							if not debounce then
								cp:Set({ CurrentValue = Lumina.CurrentTheme.Backgrounds.Groupbox }, nil, true)
							end
						end)
					end
					do
						...
						local cp = colorpickers.bg:AddColorPicker({
							IgnoreConfig = true,
							CurrentValue = Lumina.CurrentTheme.Backgrounds.Highlight,
							Callback = function(c)
								debounce = true
								Lumina.CurrentTheme.Backgrounds.Highlight = c
								themeEvent:Fire()
								task.wait(6 / 60)
								debounce = false
							end,
						...
						themeEvent.Event:Connect(function()
							if not debounce then
								cp:Set({ CurrentValue = Lumina.CurrentTheme.Backgrounds.Highlight }, nil, true)
							end
						end)
					end
				end
				...
				do
					do
						...
						local cp = colorpickers.fg:AddColorPicker({
							IgnoreConfig = true,
							CurrentValue = Lumina.CurrentTheme.Foregrounds.Dark,
							Callback = function(c)
								debounce = true
								Lumina.CurrentTheme.Foregrounds.Dark = c
								themeEvent:Fire()
								task.wait(6 / 60)
								debounce = false
							end,
						...
						themeEvent.Event:Connect(function()
							if not debounce then
								cp:Set({ CurrentValue = Lumina.CurrentTheme.Foregrounds.Dark }, nil, true)
							end
						end)
					end
					do
						...
						local cp = colorpickers.fg:AddColorPicker({
							IgnoreConfig = true,
							CurrentValue = Lumina.CurrentTheme.Foregrounds.Medium,
							Callback = function(c)
								debounce = true
								Lumina.CurrentTheme.Foregrounds.Medium = c
								themeEvent:Fire()
								task.wait(6 / 60)
								debounce = false
							end,
						...
						themeEvent.Event:Connect(function()
							if not debounce then
								cp:Set({ CurrentValue = Lumina.CurrentTheme.Foregrounds.Medium }, nil, true)
							end
						end)
					end
					do
						...
						local cp = colorpickers.fg:AddColorPicker({
							IgnoreConfig = true,
							CurrentValue = Lumina.CurrentTheme.Foregrounds.Light,
							Callback = function(c)
								debounce = true
								Lumina.CurrentTheme.Foregrounds.Light = c
								themeEvent:Fire()
								task.wait(6 / 60)
								debounce = false
							end,
						...
						themeEvent.Event:Connect(function()
							if not debounce then
								cp:Set({ CurrentValue = Lumina.CurrentTheme.Foregrounds.Light }, nil, true)
							end
						end)
					end
					do
						...
						local cp = colorpickers.fg:AddColorPicker({
							IgnoreConfig = true,
							CurrentValue = Lumina.CurrentTheme.Foregrounds.Active,
							Callback = function(c)
								debounce = true
								Lumina.CurrentTheme.Foregrounds.Active = c
								themeEvent:Fire()
								task.wait(6 / 60)
								debounce = false
							end,
						...
						themeEvent.Event:Connect(function()
							if not debounce then
								cp:Set({ CurrentValue = Lumina.CurrentTheme.Foregrounds.Active }, nil, true)
							end
						end)
					end
					do
						...
						local cp = colorpickers.fga:AddColorPicker({
							IgnoreConfig = true,
							CurrentValue = Lumina.CurrentTheme.Foregrounds.DarkHover,
							Callback = function(c)
								debounce = true
								Lumina.CurrentTheme.Foregrounds.DarkHover = c
								themeEvent:Fire()
								task.wait(6 / 60)
								debounce = false
							end,
						...
						themeEvent.Event:Connect(function()
							if not debounce then
								cp:Set({ CurrentValue = Lumina.CurrentTheme.Foregrounds.DarkHover }, nil, true)
							end
						end)
					end
					do
						...
						local cp = colorpickers.fga:AddColorPicker({
							IgnoreConfig = true,
							CurrentValue = Lumina.CurrentTheme.Foregrounds.MediumHover,
							Callback = function(c)
								debounce = true
								Lumina.CurrentTheme.Foregrounds.MediumHover = c
								themeEvent:Fire()
								task.wait(6 / 60)
								debounce = false
							end,
						...
						themeEvent.Event:Connect(function()
							if not debounce then
								cp:Set({ CurrentValue = Lumina.CurrentTheme.Foregrounds.MediumHover }, nil, true)
							end
						end)
					end
				end
				...
				do
					...
					local cp = colorpickers.divider:AddColorPicker({
						IgnoreConfig = true,
						CurrentValue = Lumina.CurrentTheme.Miscellaneous.Divider,
						Callback = function(c)
							debounce = true
							Lumina.CurrentTheme.Miscellaneous.Divider = c
							themeEvent:Fire()
							task.wait(6 / 60)
							debounce = false
						end,
					...
					themeEvent.Event:Connect(function()
						if not debounce then
							cp:Set({ CurrentValue = Lumina.CurrentTheme.Miscellaneous.Divider }, nil, true)
						end
					end)
				end
				...
				do
					do
						...
						local cp = colorpickers.shadows:AddColorPicker({
							IgnoreConfig = true,
							CurrentValue = Lumina.CurrentTheme.Miscellaneous.Shadow,
							Callback = function(c)
								debounce = true
								Lumina.CurrentTheme.Miscellaneous.Shadow = c
								themeEvent:Fire()
								task.wait(6 / 60)
								debounce = false
							end,
						...
						themeEvent.Event:Connect(function()
							if not debounce then
								cp:Set({ CurrentValue = Lumina.CurrentTheme.Miscellaneous.Shadow }, nil, true)
							end
						end)
					end
					do
						...
						local cp = colorpickers.shadows:AddColorPicker({
							IgnoreConfig = true,
							CurrentValue = Lumina.CurrentTheme.Miscellaneous.LighterShadow,
							Callback = function(c)
								debounce = true
								Lumina.CurrentTheme.Miscellaneous.LighterShadow = c
								themeEvent:Fire()
								task.wait(6 / 60)
								debounce = false
							end,
						...
						themeEvent.Event:Connect(function()
							if not debounce then
								cp:Set({ CurrentValue = Lumina.CurrentTheme.Miscellaneous.LighterShadow }, nil, true)
							end
						end)
					end
				end
					...
					do
						do
							...
							local cp = colorpickers.accent:AddColorPicker({
								IgnoreConfig = true,
								CurrentValue = Lumina.CurrentTheme.Accents.Main.Keypoints[1].Value,
								Callback = function(c)
									debounce = true
									local keypoints = Lumina.CurrentTheme.Accents.Main.Keypoints
									Lumina.CurrentTheme.Accents.Main = ColorSequence.new({
						...
						do
							...
							local cp = colorpickers.accent:AddColorPicker({
								IgnoreConfig = true,
								CurrentValue = Lumina.CurrentTheme.Accents.Main.Keypoints[2].Value,
								Callback = function(c)
									debounce = true
									local keypoints = Lumina.CurrentTheme.Accents.Main.Keypoints
									Lumina.CurrentTheme.Accents.Main = ColorSequence.new({
						...
						do
							...
							local cp = colorpickers.accent:AddColorPicker({
								IgnoreConfig = true,
								CurrentValue = Lumina.CurrentTheme.Accents.Main.Keypoints[3].Value,
								Callback = function(c)
									debounce = true
									local keypoints = Lumina.CurrentTheme.Accents.Main.Keypoints
									Lumina.CurrentTheme.Accents.Main = ColorSequence.new({
					...
					do
						do
							...
							local cp = colorpickers.accent2:AddColorPicker({
								IgnoreConfig = true,
								CurrentValue = Lumina.CurrentTheme.Accents.Brighter.Keypoints[1].Value,
								Callback = function(c)
									debounce = true
									local keypoints = Lumina.CurrentTheme.Accents.Brighter.Keypoints
									Lumina.CurrentTheme.Accents.Brighter = ColorSequence.new({
										ColorSequenceKeypoint.new(keypoints[1].Time, c),
										keypoints[2],
										keypoints[3],
									})
						...
						do
							...
							local cp = colorpickers.accent2:AddColorPicker({
								IgnoreConfig = true,
								CurrentValue = Lumina.CurrentTheme.Accents.Brighter.Keypoints[2].Value,
								Callback = function(c)
									debounce = true
									local keypoints = Lumina.CurrentTheme.Accents.Brighter.Keypoints
									Lumina.CurrentTheme.Accents.Brighter = ColorSequence.new({
						...
						do
							...
							local cp = colorpickers.accent2:AddColorPicker({
								IgnoreConfig = true,
								CurrentValue = Lumina.CurrentTheme.Accents.Brighter.Keypoints[3].Value,
								Callback = function(c)
									debounce = true
									local keypoints = Lumina.CurrentTheme.Accents.Brighter.Keypoints
									Lumina.CurrentTheme.Accents.Brighter = ColorSequence.new({
				...
				instance:CreateButton({
					...
					Callback = function()
						...
						if
							isfile(`{themesPath}/{newName.CurrentValue}{Lumina.FileSystem.FileExtension}`)
						...
						local success, returned = pcall(function()
							...
							local fullPath = `{themesPath}/{newName.CurrentValue}{Lumina.FileSystem.FileExtension}`

							local success, encoded = ThemeMethods.encodeTheme(Lumina.CurrentTheme)
							if not success then
				...
				themeEvent.Event:Connect(function()
					for key, theme in pairs(Themes) do
						if theme == Lumina.CurrentTheme then
				...
				instance:CreateButton({
					...
					Callback = function()
						if Themes[newThemeToApply] ~= nil then
							Lumina:SetTheme(Themes[newThemeToApply])
						else
							Lumina:SetTheme(
								ThemeMethods.decodeTheme(
									readfile(`{themesPath}/{newThemeToApply}{Lumina.FileSystem.FileExtension}`)
								)
							)
						end
					end,
				...
				instance:CreateButton({
					...
					Callback = function()
						...
						local success, returned = pcall(function()
							...
							local fullPath = `{themesPath}/{newThemeToApply}{Lumina.FileSystem.FileExtension}`

							local success, encoded = pcall(HttpService.JSONEncode, HttpService, Lumina.CurrentTheme)
							if not success then
								return false, "Unable to encode into JSON data"
							end
				...
				instance:CreateButton({
					...
					Callback = function()
						...
						if isfile(`{themesPath}/{newThemeToApply}{Lumina.FileSystem.FileExtension}`) then
							delfile(`{themesPath}/{newThemeToApply}{Lumina.FileSystem.FileExtension}`)
						end
			...
			function Tab:BuildConfigGroupbox(Column, Style, ButtonsCentered)
				...
				instance:CreateButton({
					...
					Callback = function()
						...
						if
							isfile(
								`{Lumina.FileSystem.Folder}/{folderpath}/configs/{inputPath.Values.CurrentValue}{Lumina.FileSystem.FileExtension}`
							)
						then
							Lumina:Notification({
						...
						local success, returned = Lumina.FileSystem:SaveConfig(
							inputPath.Values.CurrentValue,
							`{Lumina.FileSystem.Folder}/{folderpath}/configs/`
						)
						if not success then
							Lumina:Notification({
								Title = "Configuration Error",
						...
						instance.Elements["__prebuiltConfigSelector_lbl"].NestedElements["__prebuiltConfigSelector_lbl"]:Set({
							Options = Lumina.FileSystem:RefreshConfigList(
								`{Lumina.FileSystem.Folder}/{folderpath}/configs`
							),
						})
					end,
				...
				instance:CreateButton({
					...
					Callback = function()
						...
						local success, returned = Lumina.FileSystem:LoadConfig(
							selectedConfig,
							`{Lumina.FileSystem.Folder}/{folderpath}/configs/`
						)
						if not success then
				...
				local loadlabel = instance:CreateParagraph({
					...
					Content = isfile(`{Lumina.FileSystem.Folder}/{folderpath}/configs/autoload.txt`) and readfile(
						`{Lumina.FileSystem.Folder}/{folderpath}/configs/autoload.txt`
					) or "None",
				...
				instance:CreateButton({
					...
					Callback = function()
						...
						pcall(function()
							writefile(`{Lumina.FileSystem.Folder}/{folderpath}/configs/autoload.txt`, name)
						end)
						loadlabel:Set({ Content = name })
				...
				instance:CreateButton({
					...
					Callback = function()
						if isfile(`{Lumina.FileSystem.Folder}/{folderpath}/configs/autoload.txt`) then
							delfile(`{Lumina.FileSystem.Folder}/{folderpath}/configs/autoload.txt`)
						end
						loadlabel:Set({ Content = "None" })
				...
				instance:CreateButton({
					...
					Callback = function()
						...
						if
							isfile(
								`{Lumina.FileSystem.Folder}/{folderpath}/configs/{selectedConfig}{Lumina.FileSystem.FileExtension}`
							)
						then
							delfile(
								`{Lumina.FileSystem.Folder}/{folderpath}/configs/{selectedConfig}{Lumina.FileSystem.FileExtension}`
		...
		Lumina.Window.TabSections[Name] = TabSection
		return Lumina.Window.TabSections[Name]
	...
	do
		mainWindow.Content.Topbar.NotificationCenterIcon["MouseEnter"]:Connect(function()
			Tween(
				...
				{ ImageColor3 = Lumina.CurrentTheme.Foregrounds.DarkHover }
			)
		end)
		mainWindow.Content.Topbar.NotificationCenterIcon["MouseLeave"]:Connect(function()
			Tween(
				mainWindow.Content.Topbar.NotificationCenterIcon,
				{ ImageColor3 = Lumina.CurrentTheme.Foregrounds.Dark }
			)
		end)
		...
		mainWindow.Content.Topbar.NotificationCenterIcon["MouseButton1Click"]:Connect(function()
			if not notifdebounce then
				notifdebounce = true
				if Lumina.NotificationsOpen then
				...
				Lumina.NotificationsOpen = not Lumina.NotificationsOpen
				task.wait(1)
				notifdebounce = false
			end
		end)
		...
		mainWindow.Content.Topbar.Search["MouseEnter"]:Connect(function()
			Tween(mainWindow.Content.Topbar.Search, { ImageColor3 = Lumina.CurrentTheme.Foregrounds.DarkHover })
		end)
		mainWindow.Content.Topbar.Search["MouseLeave"]:Connect(function()
			Tween(mainWindow.Content.Topbar.Search, { ImageColor3 = Lumina.CurrentTheme.Foregrounds.Dark })
		end)
		...
		mainWindow.Content.Topbar.Controls.Minimize["MouseButton1Click"]:Connect(function()
			if not debounce then
				debounce = true
				Hide(mainWindow, false, true, Lumina.WindowKeybind)
				Hide(LuminaUI.Drag, false, false, Lumina.WindowKeybind)
		...
		LuminaUI.MobileToggle.MouseButton1Click:Connect(function()
			if Lumina.Minimized == true then
				if not debounce then
					debounce = true
					Unhide(mainWindow)
					Unhide(LuminaUI.Drag)
					Tween(
			...
			elseif Lumina.Minimized == false then
				if not debounce then
					debounce = true
					Hide(mainWindow, false, true, Lumina.WindowKeybind)
					Hide(LuminaUI.Drag, false, false, Lumina.WindowKeybind)
		...
		connections["__windowKeybindHidingBindConnection"] = UserInputService.InputBegan:Connect(function(input, gpe)
			...
			if input.KeyCode == Enum.KeyCode[Lumina.WindowKeybind] then
				if Lumina.Minimized == true then
					if not debounce then
						debounce = true
						Unhide(mainWindow)
	...
	return Lumina.Window
end



function Lumina.FileSystem:BuildFolderTree(FileSettings)
	...
	if FileSettings.RootFolder ~= nil then
		...
		if FileSettings.ThemesInRoot then
			paths = {
				Lumina.FileSystem.Folder,
				`{Lumina.FileSystem.Folder}/{FileSettings.RootFolder}/{FileSettings.ConfigFolder}`,
				`{Lumina.FileSystem.Folder}/{FileSettings.RootFolder}/{FileSettings.ConfigFolder}/configs`,
				`{Lumina.FileSystem.Folder}/{FileSettings.RootFolder}/themes`,
			}
		else
			paths = {
				Lumina.FileSystem.Folder,
				`{Lumina.FileSystem.Folder}/{FileSettings.RootFolder}/{FileSettings.ConfigFolder}`,
				`{Lumina.FileSystem.Folder}/{FileSettings.RootFolder}/{FileSettings.ConfigFolder}/configs`,
				`{Lumina.FileSystem.Folder}/{FileSettings.RootFolder}}/{FileSettings.ConfigFolder}/themes`,
			}
		end
	else
		...
		paths = {
			Lumina.FileSystem.Folder,
			`{Lumina.FileSystem.Folder}/{FileSettings.ConfigFolder}`,
			`{Lumina.FileSystem.Folder}/{FileSettings.ConfigFolder}/configs`,
			`{Lumina.FileSystem.Folder}/{FileSettings.ConfigFolder}/themes`,
		}
	end
...
function Lumina.FileSystem:SaveConfig(file, path)
	if isStudio or not isfile then
		return "Config system unavailable."
	end
	...
	local fullPath = `{path}{file}{Lumina.FileSystem.FileExtension}`

	local data = {
		objects = {},
	}

	for tsecidx, tabsection in next, Lumina.Window.TabSections do
...
function Lumina.FileSystem:LoadConfig(file, path)
	if isStudio or not isfile then
		return "Config system unavailable."
	end
...
function Lumina.FileSystem:RefreshConfigList(path)
	if isStudio or not isfile then
		return "Config system unavailable."
	end
...
function Lumina:LoadAutoloadConfig()
	if isStudio or not isfile then
		return "Config system unavailable."
	end

	if
		Lumina.FileSystem.AutoloadConfigPath and isfile(Lumina.FileSystem.AutoloadConfigPath .. "autoload.txt")
	then
		local name = readfile(Lumina.FileSystem.AutoloadConfigPath .. "autoload.txt")

		local success, err = Lumina.FileSystem:LoadConfig(name, Lumina.FileSystem.AutoloadConfigPath)
		if not success then
			Lumina:Notification({
...
function Lumina:SetTheme(newTheme)
	local themeToCopy = newTheme
	if type(themeToCopy) == "string" then
		themeToCopy = Lumina.Themes[themeToCopy]
	end

	Lumina.CurrentTheme = deepCopy(themeToCopy)
	themeEvent:Fire()
end

function Lumina:LoadAutoloadTheme()
	if isStudio or not isfile then
		return "Config system unavailable."
	end

	if Lumina.FileSystem.AutoloadThemePath and isfile(Lumina.FileSystem.AutoloadThemePath .. "autoload.txt") then
		local name = readfile(Lumina.FileSystem.AutoloadThemePath .. "autoload.txt")

		if Themes[name] then
			Lumina:SetTheme(name)
		else
			local content =
				readfile(Lumina.FileSystem.AutoloadThemePath .. name .. Lumina.FileSystem.FileExtension)
			local success, decoded = pcall(HttpService.JSONDecode, HttpService, content)
			if not success then
				return false, "Unable to decode JSON data."
			end

			Lumina:SetTheme(decoded)
		end
	end
end
...
Lumina:Notification({
	Title = "Enjoying Lumina?",
	Content = "Lumina Suite by iKazu",
	Duration = 10,
